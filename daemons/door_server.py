...
def create_app(credential_provider=None) -> Flask:
    """Create the door server app without starting the scheduler."""
    ...
    if not credential_provider.room_id:
        raise ValueError("Room ID is not configured in credentials.")

    door_client = DoorClient(credential_provider.room_id, jac_login)
    notification_client = DaemonNotificationClient.from_config(
        "door",
        credential_provider.ntfy_config,
        logger=app.logger,
    )
    access_token = credential_provider.door_access_token or str(uuid.uuid4())
    ...
    def get_room_label() -> str:
        ...
        try:
            ...
            return credential_provider.room_id or "unknown room"
    ...
    def log_request(response):
        if response.status_code == 404:
            app.logger.info(f'404: "{request.user_agent}" from IP {get_client_ip()}')
        return response
    ...
    app.config["door_client"] = door_client
